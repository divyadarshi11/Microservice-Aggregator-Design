...
public class TransactionReportServiceImpl implements TransactionReportService {
	...
	public TransactionDetails getTransactionDetails(String accountNumber) {
		...
		Account accountResponse = restTemplateBuilder.build().getForObject("http://localhost:8080/banking/account/getSpecificAccount/" +accountNumber , Account.class);			//Getting Account object for specific account number.
		
		List<FundTransfer> fundTrasnferResponse = restTemplateBuilder.build().getForObject("http://localhost:8081/banking/fundTransfer/getTxnForAccount/" +accountNumber, List.class);	//Getting list of fundTransfers for specific account number.
